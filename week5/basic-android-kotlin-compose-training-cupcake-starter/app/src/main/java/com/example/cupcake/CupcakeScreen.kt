...
fun CupcakeApp(modifier: Modifier = Modifier, viewModel: OrderViewModel = viewModel()) {
    ...
    Scaffold(
        ...
        NavHost( // 현재 대상을 표시하는 컨테이너 역할
            ...
            //navgraph 추가
            composable(route = CupcakeScreen.Start.name) { //각 선택할 화면
                StartOrderScreen(
            ...
            composable(route = CupcakeScreen.Flavor.name) { // 경로 이름
                val context = LocalContext.current
                SelectOptionScreen( //특정 경로에서 표시할 컴포저블
...
private fun cancelOrderAndNavigateToStart(
    ...
    navController.popBackStack(
        CupcakeScreen.Start.name, // route:돌아갈 경로,
